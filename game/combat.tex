...
\item Move [1 AP per \passus{2}]

In order to move in a strategic manner, the character has to pay 1 AP per \passus{2} moved. 



\item Run [1 AP and 1 Stamina per \passus{4}]

In order to run to a specific position, the character loses 1 stamina per \passus{4}. If two characters try to run to the same spot at the same time, an Agility-check decides who arrives first. If one of them is further away from their goal, this contestant gains a malus of 1 for every passus of difference.
